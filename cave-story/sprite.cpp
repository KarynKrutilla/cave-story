...
Sprite::Sprite(Graphics &graphics, const std::string &filePath, int sourceX, int sourceY,
	...
	if (this->_spriteSheet == NULL) {
		OutputDebugString("Error: Cannot load image!!\n");
	}
}
...
void Sprite::draw(Graphics &graphics, int x, int y) {
	SDL_Rect destinationRectangle = { x, y, this->_sourceRect.w, this->_sourceRect.h }; // where on the screen we will be drawing
